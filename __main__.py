...
from PyQt5.QtWidgets import QMainWindow,QDialog,QWidget,QAction,QApplication,QFileDialog,QTableWidgetItem,QInputDialog,QMenu,QSystemTrayIcon
...
class Main_ui_class_Ins (QMainWindow):
	...
	def open_any_file(self,*args):
		...
		if args[0]==1:
			if self.recent_Files:
				f_name_dir=self.recent_Files[-1]
		if args[0]==2:
			f_name_dir=self.recent_Files[-2]
		if args[0]==3:
			f_name_dir=self.recent_Files[-3]
		if f_name_dir :
			class_list=[]
			HeadR=[]
			max_seq=0
			...
			#Continue execution if no exception is raised
				...
				if f_name_dir in self.recent_Files:
					self.recent_Files.pop(self.recent_Files.index( f_name_dir))
					self.recent_Files.append(f_name_dir)
				else:
					self.recent_Files.append(f_name_dir)
	...
	def set_Rec_files(self): 
		...
		if self.recent_Files:
			...
			for i in self.recent_Files[-1:-1*curr_rec-1:-1]:
